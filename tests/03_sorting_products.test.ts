import { loggedJSONUserFixture, loggedEnvUserFixture } from '../fixtures';

loggedJSONUserFixture('Products are sorted by price from low to high', async ({ app }) => {
  await app.header.productSort.sortBy('Price (low to high)');
  await app.inventory.checkSortingByPrice('low to high');
});

loggedEnvUserFixture('Products are sorted by price from high to low', async ({ app }) => {
  await app.header.productSort.sortBy('Price (high to low)');
  await app.inventory.checkSortingByPrice('high to low');
});
